...
class IDTFeature(object):
    ...
    def read_IDTF_file(dir, vid_feature):
        points = []
        print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        print(os.path.join(dir, vid_feature))
        print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        # with open(os.path.join(dir,vid_feature), 'r') as f:
        # with open(('/home/vinsentp/CS221_Project/test_descriptors.txt'), 'r') as f:
        with open(('/home/vinsentp/CS221_Project/video_list.txt'), 'r') as f:
            # counter = 0
            for line in f:
                # if counter == 20: break
...
def populate_descriptors(directory, vid_features):
    ...
    for vid_feature in vid_features:
        # print(directory,vid_feature)
        vid_desc = vid_descriptors(read_IDTF_file("/home/vinsentp/CS221_Project",r"/test_descriptors.txt"))
        vid_trajs.append(vid_desc.traj)
        vid_hogs.append(vid_desc.hog)
        vid_hofs.append(vid_desc.hof)
        vid_mbhxs.append(vid_desc.mbhx)
...
def list_descriptors(directory, vid_features):
    vid_descs = []
    for vid_feature in vid_features:
      vid_descs.append(vid_descriptors(read_IDTF_file("/home/vinsentp/CS221_Project", "test_descriptors.txt")))
    return vid_descs
...
def read_IDTF_file(dir,vid_feature):
    ...
    # print(os.path.join(dir,vid_feature))
    # with open(os.path.join(dir,vid_feature), 'r') as f:
    with open(('/home/vinsentp/CS221_Project/v_ApplyEyeMakeup_g01_c01.features'), 'r') as f:
    # with open(('/home/vinsentp/CS221_Project/video_list.txt'), 'r') as f:
      #counter = 0
      for line in f:
        #if counter == 20: break
...
if __name__ == "__main__":
    dir = "/home/vinsentp/CS221_Project"
    vid_feature=r"/test_descriptors.txt"

    # list_descriptors(dir, vid_feature)

    # idtf = IDTFeature(line=read_IDTF_file(dir,vid_feature))
    # idtf.populate_descriptors("/home/vinsentp/CS221_Project", "/test_descriptors.txt" )
    # points = read_IDTF_file(dir, vid_feature)
    # print(type(points))
    # print(points)

    pop_desc = populate_descriptors("/home/vinsentp/CS221_Project", "test_descriptors.txt")
